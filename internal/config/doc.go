/*
Application behavior is defined by it's program(binary/script) and it's configurations. Thus, how to handle configurations is a taff job for most applications. In fact there are different config sources, for example, config files, commandline arguments, or environments.
...
More over, we always specify some part of configurations with config file, the other part with commandline arguments. But there is no way to specify a single configuration with multiple config source, or say, it's another taff job to handle it.
